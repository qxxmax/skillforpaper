...
A Part 2 learning run uses the quick-tier lightweight round-entry format in
`round_log.md`; the full-tier response format is for full/monitor scans.
...
Read `references/48_literature_router_playbook.md` and follow its 24 steps in
order. The playbook covers intent-mode classification (`locate` / `learn` /
`evaluate` / `cover`; when in doubt choose the highest-risk mode), the state
files per scan level, and which reference carries each gate (28 high recall,
34 channel coverage, 30 pool expansion, 35 keyword/query matrix, 36 graph
contract, 32 source links, 31 export refresh).

Non-negotiable rules enforced there:

- For broad discovery, route retrieval through
  `references/40_retrieval_backend_and_citation_expansion.md` before
  screening; a retrieval backend never assigns claim evidence or graph edges.
- Public graphs are generated from the relation ledger, not prose memory;
  when `graph_mode` selects Dijkstra, an actual executable run is mandatory
  (`scripts/run_literature_dijkstra.py`).
- No polished artifact prose until the claim/evidence ledger is current and
  the expansion plan confirms pool and facet quotas, or the gap is recorded
  as a limitation.
- Final reports cite `EvidenceID`, not just `PaperID`; run
  `scripts/validate_keyword_query_graph.py` when keyword/query/graph ledgers
  are present.
