...
def invocation() -> dict:
    return json.loads((ROOT / "invocation_manifest.json").read_text(encoding="utf-8"))
...
def metrics_payload(end_epoch: int, file_count: int, byte_count: int) -> dict:
    inv = invocation()
    freeze = json.loads((ROOT / "fresh_freeze_manifest.json").read_text(encoding="utf-8"))
    start = int(inv["start_epoch"])
    ...
    return {
        "status": "complete",
        "runtime_assignment": {
            "model": "current Codex goal-mode runtime (exact model identifier not exposed)",
            "reasoning_effort": "unavailable",
            "agent_id": "not applicable; executed in the active goal task",
            "agent_id_kind": "active_goal_task",
            "confirmation_basis": "execution mode is recorded; exact model metadata is not exposed",
            "service_tier": "unavailable",
        },
        "timing": {
            "orchestrator_start_epoch": start,
            "orchestrator_start_iso": inv["start_iso8601"],
            ...
            "fresh_freeze_epoch": freeze["freeze_epoch"],
            "fresh_freeze_elapsed_seconds": freeze["elapsed_to_freeze_seconds"],
        },
        "coverage": {
...
def validate() -> list[tuple[str, str]]:
    ...
    inv = invocation()
    assert inv["execution_mode"] == "Codex goal mode"
    assert inv["token_usage"]["status"] == "runtime_not_exposed"
    checks.append(("runtime provenance and usage policy", "PASS: goal mode recorded; exact model/token/cache unavailable and not estimated"))
...
def update_aux(payload: dict) -> None:
    ...
    (ROOT / "invocation_manifest.json").write_text(json.dumps(inv, indent=2) + "\n", encoding="utf-8")
    freeze = json.loads((ROOT / "fresh_freeze_manifest.json").read_text(encoding="utf-8"))
    start = int(inv["start_epoch"])
    timing = [
        ["event", "epoch", "iso8601", "elapsed_from_start_seconds", "note"],
        ["goal_run_start", start, inv["start_iso8601"], 0, "full skill protocol locked"],
        ["identity_confirmed", 1783865740, "2026-07-12T16:15:40+0200", 2, "arXiv identity independently recovered"],
        ["fresh_freeze", freeze["freeze_epoch"], freeze["freeze_iso8601"], freeze["elapsed_to_freeze_seconds"], "pre-comparison content freeze"],
